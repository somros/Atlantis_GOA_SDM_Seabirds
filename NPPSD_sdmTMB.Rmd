...
# Read data
```{r}
nppsd_data <- read.csv('../outputs/Diving_plankton.csv')
...
```{r}
nppsd_spde <- make_mesh(nppsd_data, c("lon", "lat"), cutoff = 20) # how do we know that this is big enough?
plot(nppsd_spde)
```
...
m_dist <- sdmTMB(
  data = nppsd_data, 
  formula = dens ~ 0 + s(distance, k = 5), #+ as.factor(year), 
  time = NULL,#"year", 
  spde = nppsd_spde, 
  reml = TRUE,
  anisotropy = FALSE,
...
write.csv(x = out, file = paste0("../outputs/",nppsd_data$name[1],"_NPPSD.csv"), row.names = FALSE)
```
...
```{r}
write.csv(x = val, file = paste0("../outputs/","validation_",nppsd_data$name[1],"_NPPSD.csv"), row.names = FALSE)
```
