...
```
The last function call above is very slow.
...
Group this by species and add up abundances by box. Then tie back to common names and then to Atlantis groups. Then add up by group by box. Should get a 436 row data frame.
```{r}
abundance_by_box <- atlantis_abundances %>% group_by(.bx0,name,common_name) %>% summarise(abundance = sum(abundance_this_box)) %>% ungroup()
...
abundance_by_box <- abundance_by_box %>% group_by(.bx0,Proposed.group) %>% summarise(abundance = sum(abundance)) %>% ungroup()
```

Join with box set again for the geometry and map.
```{r, fig.width = 12, fig.height=6}
atlantis_birds <- atlantis_box %>% select(.bx0,area,boundary) %>% left_join(abundance_by_box, by = '.bx0')
...
atlantis_birds %>% 
  ...
  ungroup() %>%
  ggplot()+
  geom_sf(aes(fill=as.numeric(abundance/area)))+
  scale_fill_viridis()+
  geom_sf(data=coast_sf, fill = 'grey')+
  coord_sf(xlim = c(atlantis_bbox$xmin, atlantis_bbox$xmax), ylim = c(atlantis_bbox$ymin, atlantis_bbox$ymax))+
  theme_bw()+
  facet_wrap(~Proposed.group, ncol=2)+
  labs(title='Seabird density from FWS Beringian Seabird database', fill = expression(paste('Density (ind ',km^-2, ')')))
```
